...
function RaceInfoCard({
  ...
  label: string; flag: string; name: string; location: string | null; date: string | null;
}) {
  return (
    <div className="db-race-card">
      <div className="db-race-card__label">{label}</div>
      <div className="db-race-card__divider" />
...
export default function DashboardHome() {
  ...
  const {
    data: driverData, isLoading: driversLoading,
    isError: driversError, error: driversErr, refetch: refetchDrivers,
  } = useQuery({
  ...
  const {
    data: teamData, isLoading: teamsLoading,
    isError: teamsError, error: teamsErr, refetch: refetchTeams,
  } = useQuery({
  ...
  const drivers: DriverStanding[] = driverData?.standings?.length
    ? driverData.standings
    : FALLBACK_DRIVER_STANDINGS;

  const teams: TeamStanding[] = teamData?.standings?.length
    ? teamData.standings
    : FALLBACK_TEAM_STANDINGS;

  const maxDriverPts = drivers.length ? Math.max(...drivers.map((d) => d.points)) : 1;
  const maxTeamPts   = teams.length   ? Math.max(...teams.map((t) => t.points))   : 1;
  ...
  // ── Resolved race cards ────────────────────────────────────────────────────
  const nextRaceFlag    = countryFlag(nextRace?.country ?? null);
  const upcomingRace    = Array.isArray(upcomingRaces) ? upcomingRaces[0] : upcomingRaces;
  const upcomingFlag    = countryFlag(upcomingRace?.country ?? null);
        ...
        <aside className="db-left-col">
          ...
          {nextLoading ? (
            ...
            <RaceInfoCard
              label="NEXT RACE"
              flag={nextRaceFlag}
              name={nextRace?.raceName ?? FALLBACK_NEXT_RACE.raceName}
              location={nextRace?.location ?? FALLBACK_NEXT_RACE.location}
              date={nextRace?.date ?? FALLBACK_NEXT_RACE.date}
            />
          )}
          ...
          {upcomingLoading ? (
            <div className="db-race-card db-race-card--skeleton">
              <div className="db-race-card__label skeleton-block" style={{ width: 100 }} />
              <div className="db-race-card__name skeleton-block" style={{ width: "80%", marginTop: 12 }} />
              <div className="skeleton-block" style={{ width: 120, marginTop: 8 }} />
            </div>
          ) : (
            <RaceInfoCard
              label="UPCOMING RACE"
              flag={upcomingFlag}
              name={upcomingRace?.race_name ?? FALLBACK_UPCOMING_RACE.race_name}
              location={upcomingRace?.circuit_name ?? FALLBACK_UPCOMING_RACE.circuit_name}
              date={upcomingRace?.date ?? FALLBACK_UPCOMING_RACE.date}
            />
          )}
        </aside>
                ...
                <div className="db-bar-chart" role="list" aria-label="Driver championship bar chart">
                  {driversLoading
                    ...
                    : drivers.map((d) => {
                        ...
                        return (
                          <div key={d.driver_id} role="listitem">
                            <BarRow
                              pos={d.position}
                              name={d.driver_name}
                              team={d.team}
                    ...
                    <tbody>
                      {driversLoading
                        ...
                        : drivers.map((d) => {
                            ...
                            return (
                              <tr key={d.driver_id} className="db-table-row">
                                <td>
